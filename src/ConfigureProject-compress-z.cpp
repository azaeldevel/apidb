...
static bool
zcompress(lzma_stream *strm, FILE *infile, FILE *outfile)
{
...
bool ConfigureProject::compress(const std::string& tarf, char * tmp_apidbDir,const std::string& filenameProj)
{
	...
	// stdin to stdout.
	bool success = init_encoder(&strm, PRESET_LEVEL);
    FILE * infile = fopen(tmp_apidbDir,"r");
    FILE * outfile = fopen(filenameProj.c_str(),"w");
	if (success)
		success = zcompress(&strm, infile, outfile);
